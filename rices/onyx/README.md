...
**Applied live on 2026-09-16 and confirmed working** — [Onyx](
...
Waybar, btop, etc. — none of that exists for COSMIC), so the `cosmic/` config
here was originally **hand-derived** from Onyx's palette rather than captured
from a live session, unlike [`milky-way`](../milky-way/README.md) and
[`osaka-jade`](../osaka-jade/README.md). It's since been copied onto a real
...
> etc.) are still the hand-derived values, not independently regenerated by
> COSMIC's own accent-color algorithm — they're confirmed to *load and
> render correctly*, not confirmed to be *exactly* what COSMIC's picker would
> have produced from the same grey. If you want to close that last gap:
> open COSMIC Settings → Appearance and nudge the accent color once — COSMIC
> will regenerate every derived shade for real.
...
| `accent` (`#8d8d8d` grey) | Ported and confirmed rendering correctly live — hand-derived hover/pressed/disabled shades (see caveat above) |
...
- `screenshots/desktop.png` — the applied rice: `Dark Street Light` wallpaper,
  pure-black top panel and dock, grey accent
- `screenshots/files-and-neovim.png` — Yazi file manager and LazyVim launching
  cleanly on top of the rice, confirming GLB's terminal tooling is unaffected
